#memory puzzle made with pygame
...
REVEALSPEED = 8 #ITEM REVEAL SPEED
...
ALLCOLORS = (RED,BLUE,GREEN,YELLOW,PURPLE,CYAN,GRAY,WHITE)
ALLSHAPES = (DONUT,OVAL,SQUARE,DIAMOND,LINES)
...
def main():
    ...
    firstSelection = None # stores (x, y) co-ordinates of first clicked box
    ...
    while True: #main game loop
        ...
        DISPSURF.fill(BGCOLOR) #draw window
        ...
        if boxx != None and boxy != None:
            ...
            if not revealedBoxes[boxx][boxy] and mouseClicked:
                ...
                else:#check if there is a match between the icons 
                    icon1shape, icon1color = getShapeAndColor(mainBoard,firstSelection[0],firstSelection[1])
                    icon2shape, icon2color = getShapeAndColor(mainBoard,boxx,boxy)

                    if icon1shape != icon2shape or icon1color != icon2color:#icons dont match, cover again
                        pygame.time.wait(1000)
                        coverBoxesAnimation(mainBoard,[(firstSelection[0], firstSelection[1]),(boxx,boxy)])
                        revealedBoxes[firstSelection[0]][firstSelection[1]] = False
                        revealedBoxes[boxx][boxy] = False
                    elif hasWon(revealedBoxes): #checks if all pairs are found 
...
def drawIcon(shape, color, boxx, boxy):
    ...
    if shape == DONUT:
        ...
        pygame.draw.circle(DISPSURF, BGCOLOR, (left + half, top + half), quarter - 5)
    elif shape == SQUARE:
